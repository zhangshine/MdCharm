...
struct footnote_info{
    struct footnote* firstOne;
    struct footnote* lastOne;
    unsigned int count;
};
...
add_footnote(
        struct footnote_info *footnoteInfo,
        const uint8_t *name, size_t name_size){
    struct footnote *fn = calloc(1, sizeof(struct footnote));
    if(!fn)
        return NULL;
    fn->hashId = hash_link_ref(name, name_size);
    fn->id = 0;
    fn->is_used = 0;
    fn->next = NULL;
    if(!footnoteInfo->firstOne){
        footnoteInfo->firstOne = fn;
    } else {
        footnoteInfo->lastOne->next = fn;
    }
    footnoteInfo->lastOne = fn;
    return fn;
}

static struct footnote *
find_footnote(struct footnote_info *footnoteHead, uint8_t *name, size_t length){
    unsigned int hash = hash_link_ref(name, length);
    struct footnote *fn = footnoteHead->firstOne;
    while(fn){
        if(fn->hashId==hash)
            return fn;
        fn = fn->next;
...
static void
free_footnotes(struct footnote_info *footnoteHead){
    struct footnote *start = footnoteHead->firstOne;
    struct footnote *next;
    while(start){
        next = start->next;
        bufrelease(start->content);
        free(start);
        start = next;
    }
    footnoteHead->firstOne = NULL;
    footnoteHead->lastOne = NULL;
    footnoteHead->count = 0;
}

static void
free_single_footnote(struct footnote *fn){
    bufrelease(fn->content);
    free(fn);
    fn = NULL;
}
...
char_link(struct buf *ob, struct sd_markdown *rndr, uint8_t *data, size_t offset, size_t size)
{
	...
	while (i < size && _isspace(data[i]))
		i++;
    /* footnote link style */
    if ((rndr->ext_flags & MKDEXT_FOOTNOTE) && rndr->cb.footnote_link &&
            i <= size && size>3 && data[1]=='^'){
        struct footnote *fn=NULL;
        struct buf id = { 0, 0, 0, 0};
        id.data = data+1;
        id.size = txt_e - 1;
        fn = find_footnote(&rndr->fnInfo, id.data, id.size);
        if(fn && !fn->is_used){
            fn->id = ++rndr->fnInfo.count;
            fn->is_used = 1;
            ret = rndr->cb.footnote_link(ob, fn->id);
            goto cleanup;
        }
    }
	/* inline style link */
...
parse_paragraph(struct buf *ob, struct sd_markdown *rndr, uint8_t *data, size_t size)
{
	...
	if (!level) {
        ...
        if((rndr->ext_flags & MKDEXT_HEADER_ID_ATTRIBUTE) && work.size>3){
            size_t idStart=0, idEnd;
            for(; (idStart+3)<work.size&&!(work.data[idStart]=='{'&&work.data[idStart+1]=='#'); idStart++);
            if( (idStart+3)<work.size && work.data[idStart]=='{' && work.data[idStart+1]=='#'){
                for(idEnd=idStart+2; idEnd<work.size&&work.data[idEnd]!='}'; idEnd++);
                if(idEnd<work.size){
                    work.size = idStart;
                    idStart = idStart + 2;
                    id = rndr_newbuf(rndr, BUFFER_SPAN);
                    bufput(id, work.data+idStart, idEnd-idStart);
                }
            }
        }
		...
		if (rndr->cb.header)
            rndr->cb.header(ob, header_work, id, (int)level, rndr->opaque);
...
parse_atxheader(struct buf *ob, struct sd_markdown *rndr, uint8_t *data, size_t size)
{
	size_t level = 0;
	size_t i, end, skip;
    struct buf *id = NULL;//for header id attribute
    ...
    //header id attribute
    if( (rndr->ext_flags & MKDEXT_HEADER_ID_ATTRIBUTE)
            && ((i+3)<end) ){
        size_t idStart, idEnd;
        for(idStart=i; (idStart+3)<end&&!(data[idStart]=='{'&&data[idStart+1]=='#'); idStart++);
        if( (idStart+3)<end ){
            for(idEnd=idStart+2; (idEnd+1)<end&&data[idEnd]!='}'; idEnd++);
            if(idEnd<end){
                end = idStart;
                idStart = idStart + 2;
                id = rndr_newbuf(rndr, BUFFER_SPAN);
                bufput(id, data+idStart, idEnd-idStart);
            }
        }
    }
	...
	if (end > i) {
		...
		if (rndr->cb.header)
            rndr->cb.header(ob, work, id, (int)level, rndr->opaque);
...
/* is_footnote -> returns whether a line is a footnote or not, similar with is_ref */
static int
is_footnote(const uint8_t *data, size_t beg, size_t end, size_t *last,
    ...
    i += beg;/* offset of the whole document */

    /* id part: anything prefix with `^` but a newline between brackets */
    if (data[i] != '[') return 0;
    i++;
    idStart = i;
    if (idStart>=end || data[idStart]!='^') return 0; // footnote flag
    while( i<end && data[i] != '\n' && data[i] != '\r' && data[i] != ']')
        i++;
    if ( i>= end || data[i] != ']') return 0;
    idEnd = i;
    if( (idEnd-idStart)<2 )//at last two char, one is '^', the other is name
        return 0;

    /* spacer: colon (space) * newline? (space){4} */
    i++;
    if (i >= end || data[i] != ':') return 0;
    i++;
    ...
    if (i < end && (data[i] == '\n' || data[i] == '\r')){//unix/mac
        ...
        if ( i<end && data[i] == '\r' && data[i-1] == '\n')//win
            i++;
        //better align, leave the frist line of the footnote empty
        if(i+1>=end) return 0;
        if(data[i]=='\t'){
            i++;
        } else {
        ...
        contentStart = i;
    } else { // footenote following `:`, i>=end or i is the start of content
        if (i>=end) return 0;
        contentStart = i;
    }
    i++;
    ...
    if(last)
        *last = line_end;
    if(md){
        size_t size = contentEnd;
        size_t begin = contentStart, cend;
        struct buf *mdContent = bufnew(idEnd-idStart);
        struct footnote *fn = add_footnote(&md->fnInfo, data + idStart, idEnd - idStart);
        if (!fn)
            return 0;

        while(begin<size){
            cend = begin;
            while(cend<size && data[cend]!='\n'&&data[cend]!='\r')
                cend++;
            if (cend>begin)
                expand_tabs(mdContent, data+begin, cend-begin);
            while(cend<size && (data[cend]=='\n'||data[cend]=='\r')){
                if (data[cend]=='\n'||(cend+1<size && data[cend+1]!='\n'))
                    bufputc(mdContent, '\n');
                cend++;
            }
            if(cend+1<size && data[cend]=='\t'){
                cend += 1;
            }else if(cend+3<size&& data[cend]==' ' && data[cend+1]==' ' && data[cend+2]==' ' && data[cend+3]==' ')
                cend += 4;
            begin=cend;
        }
        fn->content = mdContent;
    }
    return 1;
}
...
#define MARKDOWN_GROW(x) ((x) + ((x) >> 1))
    ...
    /* reset the footnote list */
    md->fnInfo.firstOne = NULL;
    md->fnInfo.lastOne = NULL;
    md->fnInfo.count = 0;
	...
	while (beg < doc_size) /* iterating over lines */
        if ((md->ext_flags & MKDEXT_FOOTNOTE) &&(!fence_code_area)&& is_footnote(document, beg, doc_size, &end, md))
            beg = end;
        else if ((!fence_code_area)&&is_ref(document, beg, doc_size, &end, md->refs))
			beg = end;
		else { /* skipping to the next line */
            if((md->ext_flags & MKDEXT_FOOTNOTE) && (is_codefence(document+beg, doc_size-beg, NULL)!=0)){
                fence_code_area ^= 1;
            }
			end = beg;
    ...
    //footnote bottom content
    if (md->cb.footnote_bottom_content_list_item && md->cb.list && (md->ext_flags&MKDEXT_FOOTNOTE) && md->fnInfo.firstOne){
        struct buf *footnoteListItems = rndr_newbuf(md, BUFFER_BLOCK);
        unsigned int i=0;
        for(i=1; i<=md->fnInfo.count; i++){
            struct footnote *prev = NULL;
            struct footnote *f = md->fnInfo.firstOne;
            while(f){
                if(f->id == i){
                    if(f->content->size){
                        struct buf *mdHtml = rndr_newbuf(md, BUFFER_BLOCK);
                        if(mdHtml){
                            bufprintf(f->content, "<a href=\"#fnref:%d\" rev=\"footnote\">&#8617;</a>\n", f->id);
                            parse_block(mdHtml, md, f->content->data, f->content->size);
                            md->cb.footnote_bottom_content_list_item(footnoteListItems, f->id, mdHtml);
                            rndr_popbuf(md, BUFFER_BLOCK);
                        }
                    }
                    if(f==md->fnInfo.firstOne){
                        md->fnInfo.firstOne = f->next;
                    } else {
                        assert(prev);
                        prev->next = f->next;
                    }
                    free_single_footnote(f);
                    break;
                }
                prev = f;
                f = f->next;
        ...
        bufputs(ob, "<div class=\"footnotes\"><hr />");
        md->cb.list(ob, footnoteListItems, MKD_LIST_ORDERED, NULL);
        rndr_popbuf(md, BUFFER_BLOCK);
        bufputs(ob, "</div>");
    }
